...
| [snippets/antique-vellum-scriptorium.json](snippets/antique-vellum-scriptorium.json) | Option 1: Scriptorium |
...
| [snippets/antique-vellum-scriptorium.css](snippets/antique-vellum-scriptorium.css) | Core snippet for Option 1: font isolation, font overrides, and typography tuning |
...
| [snippets/antique-vellum-scriptorium.json](snippets/antique-vellum-scriptorium.json) | 方案一：Scriptorium |
...
| [snippets/antique-vellum-scriptorium.css](snippets/antique-vellum-scriptorium.css) | Option 1 核心 snippet：语种隔离、字体覆盖与排版微调 |
...
4. Depending on the preset you want to use, open one of the following files:
   - `snippets/antique-vellum-scriptorium.json`
...
For **Option 1: Scriptorium**, `QiushuiShotai` must be installed as a **system font**. The snippet references it via `local("QiushuiShotai")`; it is not loaded from a local `.ttf` file inside the vault. If you want the current article-title look, also install `Hiragino Sans GB` so inline titles can switch to that display face.
...
对于 **Option 1: Scriptorium**，`QiushuiShotai` 必须安装为**系统字体**。对应 snippet 通过 `local("QiushuiShotai")` 调用它，而不是从 vault 内部的 `.ttf` 文件直接加载。如果你想得到当前这版文章标题效果，也请安装 `Hiragino Sans GB`，让 inline title 能切换到这颗展示字体。
...
| `QiushuiShotai` | Scriptorium primary CJK source |
| `Hiragino Sans GB` | Scriptorium inline-title font |
...
| `QiushuiShotai` | Scriptorium 主 CJK 字体来源 | 详见字体作者发布渠道 |
| `Hiragino Sans GB` | Scriptorium 文章标题字体 | 详见字体作者发布渠道 |
...
|---|---|---|
| `Athelas`, `Palatino` | Scriptorium Latin text | Built into macOS |
| `STKaiti`, `Kaiti SC`, `KaiTi` | Scriptorium CJK fallback | Depends on system environment |
| `Hiragino Sans GB` | Scriptorium inline-title family | Verify local installation |
...
| 字体 | 当前用途 | 备注 |
|---|---|---|
| `Athelas`, `Palatino` | Scriptorium 西文正文 | macOS 原生内置 |
| `STKaiti`, `Kaiti SC`, `KaiTi` | Scriptorium CJK 后备字体 | 取决于系统环境 |
| `Hiragino Sans GB` | Scriptorium 文章标题字体族 | 建议本地确认 |
...
1. **Font routing and font overrides** — The snippet injects the actual font stack and local title font behavior for each preset. At the moment, **Option 1: Scriptorium and Option 3: Feuilleton explicitly implement CJK font isolation via `@font-face` + `unicode-range`**; Option 2: Codex still relies on a normal font stack.
...
1. **字体分流与字体覆盖** —— snippet 会注入各方案真实使用的字体栈，以及局部标题字体行为。目前，**Option 1: Scriptorium 与 Option 3: Feuilleton 都明确通过 `@font-face` + `unicode-range` 实现了 CJK 语种隔离**；Option 2: Codex 仍然依赖普通 font stack。
...
1. Copy the matching snippet into your vault's `.obsidian/snippets/` folder:
   - Option 1: `snippets/antique-vellum-scriptorium.css`
